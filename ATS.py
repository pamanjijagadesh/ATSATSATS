...
# IMPORTANT: use secrets in Streamlit Cloud!
AZURE_ENDPOINT = st.secrets["AZURE_ENDPOINT"]
AZURE_CREDENTIAL = st.secrets["AZURE_CREDENTIAL"]
...
def initialize_llm_and_pack():
    ...
    try:
        with st.status("Initializing AI Resources...", expanded=True) as status:
            ...
            # Direct pack import (no download)
...
async def process_single_resume_async(uploaded_file, job_description, criteria, ResumeScreenerPack, llm, temp_dir):
    ...
    try:
        ...
        return {
            ...
            "Overall Reasoning": f"Error during processing: {e}"
        }
...
async def run_screening_async(uploaded_files, job_description, criteria, ResumeScreenerPack, llm, progress_bar_placeholder):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for future in asyncio.as_completed(tasks):
            ...
            completed_tasks += 1
            percent = int((completed_tasks / total_files) * 100)
            progress_bar_placeholder.progress(
                percent,
                text=f"Processing resumes... {completed_tasks}/{total_files} ({percent}%)"
            )
...
if st.button("Screen Resumes Concurrently", type="primary"):
    if not job_description or not criteria or not uploaded_files:
        st.warning("Please fill all fields and upload resumes.")
        st.stop()
    ...
    with st.status("Running Concurrent Screening...", expanded=True) as status:
        try:
            results_data = asyncio.run(
                run_screening_async(uploaded_files, job_description, criteria, ResumeScreenerPack, llm, progress_bar)
            )
            status.update(label=f"Completed! ({len(results_data)} resumes processed)", state="complete")
        except Exception as e:
    ...
    if results_data:
        ...
        df = df[
            ["Resume Name", "ATS Score (%)", "ATS Overall Decision", "Overall Reasoning"]
        ]
